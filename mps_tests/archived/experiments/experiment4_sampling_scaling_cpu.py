...
# Experiment 4: Sampling Scaling (CPU)
...
output_file = 'experiment4_sampling_scaling_cpu.jsonl'
num_trials = 1  # One trial per configuration as requested ("try to estimate... by fixing various circuit settings")
...
if os.path.exists(output_file):
    ...
    with open(output_file, 'w') as f:
        f.write(f'# Experiment 4: Sampling Scaling (CPU)\n')
...
for num_qubits in qubit_counts:
    for depth in depths:
        for bond_dimension in bond_dimensions:
            ...
            # We want to vary shots for the SAME circuit to estimate A + b*shots
            
            # Using a fixed seed per configuration ensures we are testing the "same" circuit structure 
            # even if we regenerate it in the loop (though here we generate it once per config block ideally, 
            # but to match loop structure we can generate it inside trials if trials > 1. 
            # Since trials=1, we just generate it here).
...
print(f'Experiment 4 (CPU) complete! Results written to {output_file}')
