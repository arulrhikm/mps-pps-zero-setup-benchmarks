...
# Experiment 1: Qubit Scaling (GPU) - Rerun
...
output_file = os.path.join(
    ...
    "..", "..", "data", "gpu",
    "experiment1_qubit_scaling_gpu.jsonl",
)
num_trials = 4
...
for num_qubits in range(min_qubits, max_qubits + 1, 4):
    for trial in range(1, 1 + num_trials):  # trials 1 to 4
        if (num_qubits, trial) in completed_runs:
            print(f"Skipping (qubits={num_qubits}, trial={trial}) - already completed")
            continue
...
print(f'Experiment 1 (GPU) rerun complete! Results written to {output_file}')
