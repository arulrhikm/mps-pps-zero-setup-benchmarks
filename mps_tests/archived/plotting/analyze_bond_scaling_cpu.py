...
# Load CPU data
data_file = 'c:/Users/arulr/Projects/BlueQubit/mps_tests/cpu/experiment2_bond_scaling_cpu_updated.jsonl'
bond_dims, runtimes = [], []
...
print(f"CPU: T ~ chi^{p:.4f}  (slope p = {p:.4f}, R² = {r2:.4f}, fit for chi >= 40)")
...
plt.title(f'MPS Bond Scaling (CPU)\n$T \\propto \\chi^{{{p:.2f}}}$, $R^2 = {r2:.4f}$ (fit for $\\chi \\geq 40$)', fontsize=14)
plt.legend(fontsize=11)
plt.grid(True, ls='--', alpha=0.7)
plt.tight_layout()
plt.savefig('experiment2_bond_scaling_cpu_loglog.png')
print("Saved: experiment2_bond_scaling_cpu_loglog.png")
