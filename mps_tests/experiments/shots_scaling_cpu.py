#!/usr/bin/env python3
"""
MPS shots scaling (CPU) — fixed QV circuit, sweep shots.

Circuit: n=40, depth=10, χ=256 (same family as bond-dimension plots).
Shots: {1, 10, 100, 1000, 5000, 10000}  |  5 trials per (shots, circuit seed).
Records run_time_ms, mps_build_time_ms, sampling_time_ms for build vs sampling plots.

Device: mps.cpu

Usage:
    python experiments/shots_scaling_cpu.py
    # Default output: data/shots_scaling_cpu.jsonl
"""
...
DEFAULT_OUTPUT = os.path.join(DATA_DIR, "shots_scaling_cpu.jsonl")

NUM_QUBITS = 40
DEPTH = 10
BOND_DIMENSION = 256
SHOT_VALUES = [1, 10, 100, 1000, 5000, 10000]
NUM_TRIALS = 5
...
def is_completed_run(row, *, depth, bond_dimension):
    ...
    while pending_jobs:
        ...
        for job in refreshed:
            ...
            if job.run_status == "COMPLETED" and is_completed_run(
                ...
                print(
                    f"Completed (shots={meta['shots']}, trial={meta['trial']}) "
                ...
                print(
                    f"FAILED (shots={meta['shots']}, trial={meta['trial']}) "
...
def run_benchmark(
    ...
    if os.path.exists(output_file):
        ...
        with open(output_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    r = json.loads(s)
                    if is_completed_run(r, depth=depth, bond_dimension=bond_dimension):
                        completed_runs.add((r["shots"], r["trial"]))
                except (json.JSONDecodeError, KeyError):
                    continue
        print(f"Found {len(completed_runs)} completed runs")

    total = len(shot_values) * num_trials
    remaining = total - len(completed_runs)

    print(f"\n{'='*60}")
    print("MPS CPU — shots scaling")
    print(
        f"n={NUM_QUBITS}  |  depth={depth}  |  χ={bond_dimension}  |  "
        f"trials={num_trials}"
    )
    print(f"shots: {shot_values}")
    ...
    for shots in shot_values:
        for trial in range(num_trials):
            if (shots, trial) in completed_runs:
                print(f"  Skipping (shots={shots}, trial={trial}) — already done")
                continue
    ...
    while config_queue or pending_jobs:
        while config_queue and len(pending_jobs) < max_in_flight:
            ...
            print(
                f"Submitting (shots={meta['shots']}, trial={meta['trial']})... ",
                end="",
                flush=True,
            )
            try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MPS CPU shots scaling")
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=SHOT_VALUES,
        help="Shot counts to benchmark (default: 1 10 100 1000 5000 10000)",
    )
