...
fig7_pps_gpu.py
===============
Reproduce Fig. 7 — Convergence of ⟨Z_62⟩ with fixed θ_X = 0.4.

Circuit (Eq. 10):
  - 127 qubits, IBM heavy-hex lattice
  - θ_ZZ = -π/2
  - θ_X  = 0.4  (fixed, same angle every step and every qubit)
  - Observable ⟨Z_62⟩
...
The two rows of Fig 7 correspond to plotting subsets of this data
with different convergence thresholds:
  Top row:    ε_tol = 0.01
  Bottom row: ε_tol = 0.001
...
rzz_angle      = -np.pi / 2
rx_angle       = 0.4                          # fixed θ_X for Fig 7

# Trotter step counts from Fig 7
...
for T in trotter_steps:
    ...
    print(f"\n{'='*60}")
    print(f"T = {T}  ({num_ops} gates),  θ_X = {rx_angle}")
    print(f"{'='*60}")
